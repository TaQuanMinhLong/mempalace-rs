...
impl GeneralExtractor {
    ...
    fn split_by_turns(&self, lines: &[&str], turn_patterns: &[Regex]) -> Vec<String> {
        let mut segments = Vec::new();
...
mod tests {
    ...
    fn test_extract_milestone() {
        let extractor = GeneralExtractor::new();
        let text =
            "It works! After three days of debugging, I finally got the authentication working.";
        let memories = extractor.extract(text, 0.3);
    ...
    fn test_split_by_turns() {
        let extractor = GeneralExtractor::new();
        let lines = vec!["Human: Hello", "Assistant: Hi there", "Human: How are you?"];
        ...
        let segments = extractor.split_by_turns(&lines, &turn_patterns);
    ...
    fn test_extract_with_high_min_confidence() {
        ...
        // High confidence threshold may filter out this memory
