...
mod tests {
    ...
    fn test_extract_candidates() {
        let extractor = EntityExtractor::new();
        let text =
            "Alice said hello to Bob. Alice told Bob something. Alice wrote a book about Bob.";
        let candidates = extractor.extract_candidates(text);

        assert!(candidates.contains_key("Alice"));
        assert!(candidates.contains_key("Bob"));
        // "Alice" appears 3 times, "Bob" appears 3 times
    ...
    fn test_classify_project() {
        let extractor = EntityExtractor::new();
        let text =
            "We built MemPalace v2. We shipped MemPalace. The MemPalace architecture is great.";
        let candidates = extractor.extract_candidates(text);
    ...
    fn test_cross_file_entity_aggregation() {
        // Verify that entity frequency is aggregated across multiple files
        let extractor = EntityExtractor::new();

        let file1 = tempfile::NamedTempFile::with_suffix(".txt").unwrap();
        let file2 = tempfile::NamedTempFile::with_suffix(".txt").unwrap();

        // Alice appears 3+ times with person signals (dialogue markers, actions)
        // This tests that frequency accumulates across files before classification
        std::fs::write(
            file1.path(),
        ...
        let result =
            extractor.detect_from_files(&[file1.path().as_ref(), file2.path().as_ref()], 10);

        // Alice appears 5 times total across both files
        // It should be detected (as person or uncertain) since frequency >= 3
