...
mod tests {
    ...
    fn test_truncate_safe_utf8() {
        // Test with em dashes (3 bytes each)
        let s = "Hello ── world";
    ...
    fn test_truncate_safe_utf8_boundary() {
        ...
        // If we truncated at byte 199 instead of char boundary, it would panic
        let s = "┌───────────────────────────────────────────────────────────────────────────────┐";
        // This string is ~90 chars, ~270 bytes
        let result = truncate_safe(s, 50);
        // Should not panic and should have valid char count
        assert!(result.chars().count() <= 50);
    }

    #[test]
    fn test_truncate_safe_utf8_mixed() {
        // Mix of ASCII, Chinese, emoji
        let s = "Hello 世界! 🎉";
        let result = truncate_safe(s, 5);
        assert!(result.chars().count() <= 5);
    }
