...
mod tests {
    ...
    fn test_chunk_file_basic() {
        // Content must be >= MIN_CHUNK_SIZE (50 chars) to produce chunks
    ...
    fn test_chunk_file_preserves_words() {
        // Content must be >= MIN_CHUNK_SIZE (50 chars)
    ...
    fn test_chunk_file_exact_size() {
        ...
        let chunks = FileMiner::chunk_file(&content, 800);
        assert!(chunks.len() >= 2); // Should have at least 2 chunks due to overlap
    }

    #[test]
    fn test_chunk_file_utf8_char_boundary() {
        // Test with multi-byte UTF-8 characters (em dash is 3 bytes)
        // This should NOT panic - was causing "end byte index is not a char boundary"
    ...
    fn test_chunk_file_utf8_box_drawing() {
        // Box drawing characters (─, │, ┌, etc.) are 3 bytes in UTF-8
        // These were causing panics in the original code
        let content =
            "┌─────────────────────┐\n│ Test Content Here │\n└─────────────────────┘\n".repeat(20);
        let chunks = FileMiner::chunk_file(&content, 200);
        assert!(!chunks.is_empty());
        // Verify chunks are valid UTF-8 strings
        for chunk in &chunks {
            let _ = chunk.as_str(); // This would fail if not valid UTF-8
        }
    }

    #[test]
    fn test_chunk_file_utf8_various() {
        // Mix of various UTF-8 characters: Chinese, emoji, arrows, etc.
        let content =
            "Hello 世界! 🎉 → ← ↑ ↓ ● ○ ◆ ◇\n中文测试\nEm dash — and en dash –\n".repeat(30);
    ...
    fn test_room_detection_by_content() {
        ...
        let content =
            "I am scared and worried about my feelings. The consciousness of being aware.";

        let room = miner.detect_room(&file_path, content, project_path);
        // Content has "scared", "worried", "consciousness" keywords
    ...
    fn test_room_detection_utf8_char_boundary() {
        // Test detect_room with content containing UTF-8 chars
        // The bug was at content[..content.len().min(2000)] which could panic
        let config = Config::default();
        let storage = ChromaStorage::new(Path::new("test"), "test").unwrap();
        let miner = FileMiner::new(config, storage).unwrap();
        ...
        // Create content with box drawing chars that spans > 2000 bytes but < 2000 chars
        // Each box char is 3 bytes. This content is ~150 chars but ~450 bytes
        let content = "┌──────────────────────────────────────────────┐\n│ Test file with UTF-8 box drawing characters │\n└──────────────────────────────────────────────┘\n"
            .repeat(10);

        // This should NOT panic - was causing "end byte index 2000 is not a char boundary"
        let room = miner.detect_room(&file_path, &content, project_path);
    ...
    fn test_room_detection_utf8_mixed() {
        ...
        // Mix of Chinese chars, emoji, special chars
        let content =
            "这是一个测试文件 📝\n中文内容测试\nRust编程语言 🔧\n—— 破折号 ——\n".repeat(50);
    ...
    fn test_extract_entities_basic() {
        ...
        // Should find Alice (appears 3 times) and Bob (appears 2 times)
        assert!(entities.contains(&"Alice".to_string()));
        // Bob only appears 2 times, might not be extracted depending on threshold
    }
    ...
    fn test_extract_entities_filters_common_words() {
        ...
        // "The" and "dog" are common words or appear too frequently as parts
    ...
    fn test_is_likely_entity() {
        ...
        assert!(!FileMiner::is_likely_entity("ab")); // too short
    }
}
